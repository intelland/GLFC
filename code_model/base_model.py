...
class BaseModel(ABC):
    ...
    def __init__(self, config):
        ...
        if self.isTrain == True:
            ...
            if config["results"].get("test_model_dir") == None:
               
                experiment_folder = util.find_latest_experiment(config["name"])
                test_model_dir = os.path.join(experiment_folder,"models")
                self.save_dir = test_model_dir
            else:
                self.save_dir = os.path.join(config["results"]["test_model_dir"])
    ...
    def setup(self, config):
        ...
        if not self.isTrain :
            load_suffix = config["results"]["test_epoch"]
            print("=====================load network=====================")
            self.load_networks(load_suffix)
        self.print_networks(config["record"]["verbose"])
    ...
    def print_networks(self, verbose):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if verbose:
                    log_lines.append(str(net) + '\n')
                log_lines.append('[Network %s] Total number of parameters : %.3f M\n' % (name, num_params / 1e6))
        log_lines.append('-----------------------------------------------\n')
