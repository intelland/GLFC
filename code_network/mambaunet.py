...
from code_network.modules.transformer import PatchEmbed
...
class MambaUnet(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=1, depths=[2, 2, 9, 2], 
                 dims=[96, 192, 384, 768], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 mamba_norm_layer=nn.LayerNorm, patch_norm=True,
        ...
        self.patch_embed = PatchEmbed2D(patch_size=patch_size, in_chans=in_chans, embed_dim=self.embed_dim,
            mamba_norm_layer=mamba_norm_layer if patch_norm else None)
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer ==0 :
                layer_up = PatchExpand(dim=int(self.embed_dim * 2 ** (self.num_layers-1-i_layer)), dim_scale=2, norm_layer=mamba_norm_layer)
            else:
        ...
        self.norm = mamba_norm_layer(self.num_features)
        self.norm_up = mamba_norm_layer(self.embed_dim)
